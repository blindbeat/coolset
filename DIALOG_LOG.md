...
- Split the typography to match the Figma reference, which uses SF Pro Text for chrome (title, column headers, the section filter button) and Inter for data. Added a `--font-label` token mapped to the system font stack rather than shipping a webfont — SF Pro isn't web-licensed, so the stack resolves to it natively on Apple devices and falls through to each platform's own UI font (Segoe UI on Windows, Roboto on Android) elsewhere. No font download, no licensing risk, and the design feels native everywhere.
